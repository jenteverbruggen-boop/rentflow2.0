...
export function MaterialSplitEditor({
  ...
  const removeOne = useMutation({
    mutationFn: (assignmentId: number) =>
      fetch(`/api/periods/${period.id}/materials/${assignmentId}`, {
        method: "DELETE",
      }),
  ...
  const removeBundle = useMutation({
    mutationFn: (bundleBookingId: number) =>
      fetch(`/api/periods/${period.id}/bundles/${bundleBookingId}`, {
        method: "DELETE",
      }),
    onSuccess: invalidate,
  });
                          ...
                          <div className="space-y-1 mt-1 ml-4">
                            {items.map((m) => {
                              ...
                              return (
                                <div
                                  key={m.material.id}
                                  className="flex flex-wrap items-center gap-x-1.5 gap-y-1 bg-muted/30 rounded px-2 py-1.5 text-xs"
                                >
                                  ...
                                  <div className="flex items-center gap-1.5 shrink-0">
                                    ...
                                    <Button
                                      ...
                                      className="h-7 w-7"
                                      disabled={
                                        add.isPending || qty > m.availableCount
                                      }
                                      onClick={() =>
                                        add.mutate({
            ...
            <div className="p-3 md:p-0">
              <ScrollArea className="h-[400px] pr-2">
                <div className="space-y-2">
                          ...
                          <div className="space-y-1 mt-1 ml-4">
                            {items.map((g) => {
                              ...
                              return (
                                <div
                                  key={g.key}
                                  className="flex items-center gap-1.5 bg-muted/40 rounded px-2 py-1 text-xs"
                                >
                                  <Button
                                    size="icon"
                                    variant="outline"
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-[10px] text-muted-foreground">
                                      {g.units} × {formatEUR(perUnit)} ={" "}
                                      {formatEUR(perUnit * g.units)}
                                    </p>
                                  </div>
                                  <Button
                                    size="icon"
                  ...
                  {groups.length === 0 &&
                    (period.bundleBookings ?? []).length === 0 && (
                      <p className="text-xs text-muted-foreground py-6 text-center">
                  ...
                  {(period.bundleBookings ?? []).map((b) => (
                    <div
                      key={`bundle-${b.id}`}
                      className="flex items-center gap-1.5 bg-blue-950/30 rounded px-2 py-1 text-xs border border-blue-800/30"
                    >
                      <div className="flex-1 min-w-0">
                        <p className="font-medium truncate flex items-center gap-1">
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          ...
                          {formatEUR(b.dayPriceSnapshot * days)} totaal
                        </p>
                      </div>
                      <Button
                        size="icon"
                        ...
                        onClick={() => removeBundle.mutate(b.id)}
