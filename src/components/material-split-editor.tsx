...
export function MaterialSplitEditor({ period, project, onWarnings, onError }: Props) {
  ...
  const removeBundle = useMutation({
    mutationFn: (bundleBookingId: number) =>
      fetch(`/api/periods/${period.id}/bundles/${bundleBookingId}`, { method: "DELETE" }),
    onSuccess: invalidate,
  });
                              ...
                              <div key={m.material.id} className="flex flex-wrap items-center gap-x-1.5 gap-y-1 bg-muted/30 rounded px-2 py-1.5 text-xs">
                                <div className="flex-1 min-w-[120px]">
                                  <p className="font-medium truncate flex items-center gap-1">
                                    {m.material.name}
                                    {m.material.isBundle && <Badge variant="secondary" className="text-[9px] h-3.5 px-1">set</Badge>}
                                  </p>
                                  <p className="text-[10px] text-muted-foreground truncate">
                                    {m.availableCount} {m.material.isBundle ? "set(s) vrij" : `/${m.totalStock} vrij`} · {formatEUR(m.material.dayPrice)}/d
                                  </p>
                                </div>
                                <div className="flex items-center gap-1.5 shrink-0">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => add.mutate({ materialId: m.material.id, quantity: m.material.isBundle ? 1 : qty })}
          ...
          <section className="rounded-lg border border-border overflow-hidden md:rounded-none md:border-0 md:overflow-visible md:space-y-2">
            ...
            <ScrollArea className="h-[400px] pr-2">
              <div className="space-y-2">
                ...
                {(period.bundleBookings ?? []).map((b) => (
                  <div key={`bundle-${b.id}`} className="flex items-center gap-1.5 bg-blue-950/30 rounded px-2 py-1 text-xs border border-blue-800/30">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate flex items-center gap-1">
                        {b.material?.name ?? "Bundle"}
                        <Badge variant="secondary" className="text-[9px] h-3.5 px-1">set ×{b.quantity}</Badge>
                      </p>
                      <p className="text-[10px] text-muted-foreground">
                        {formatEUR(b.dayPriceSnapshot)}/d · {formatEUR(b.dayPriceSnapshot * days)} totaal
                      </p>
                    </div>
                    <Button size="icon" variant="ghost" className="h-7 w-7 hover:text-destructive" onClick={() => removeBundle.mutate(b.id)} title="Set verwijderen">
                      <Trash2 className="h-3.5 w-3.5" />
                    </Button>
                  </div>
                ))}
