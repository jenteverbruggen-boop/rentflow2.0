...
export function PlanningCalendarItem({
  project,
  people,
  materials,
}: PlanningCalendarItemProps) {
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <div className="flex gap-2 mt-0.5">
            {people > 0 && (
              <span className="text-xs opacity-70">👥 {people}</span>
            )}
            {materials > 0 && (
              <span className="text-xs opacity-70">📦 {materials}</span>
            )}
          </div>
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-64 p-3 space-y-2" align="start">
        ...
        <div className="text-xs text-muted-foreground space-y-0.5">
          ...
          <p>
            👥 {people} personen · 📦 {materials} materialen
          </p>
        </div>
        <Link
