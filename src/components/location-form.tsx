...
export function LocationForm({ open, onOpenChange, defaultValues, onSubmit, isPending }: Props) {
  const form = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: { name: "" } });

  useEffect(() => {
    form.reset(defaultValues ? {
      name: defaultValues.name, address: defaultValues.address ?? "", postalCode: defaultValues.postalCode ?? "",
      city: defaultValues.city ?? "", phone: defaultValues.phone ?? "", notes: defaultValues.notes ?? "",
    } : { name: "" });
  }, [defaultValues, form]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader><DialogTitle>{defaultValues ? "Locatie bewerken" : "Nieuwe locatie"}</DialogTitle></DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
            <FormField control={form.control} name="name" render={({ field }) => (
              <FormItem><FormLabel>Naam *</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>
            )} />
