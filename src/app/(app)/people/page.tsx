...
export default function PeoplePage() {
  ...
  const upsert = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["people"] }); setOpen(false); setEditing(null); },
  });

  const remove = useMutation({
    mutationFn: async (id: number) => { await fetch(`/api/people/${id}`, { method: "DELETE" }); },
  ...
  return (
    <div className="space-y-6">
                ...
                <div>
                  ...
                  {p.functions && p.functions.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {p.functions.map((f) => <Badge key={f.id} variant="secondary" className="text-xs">{f.name}</Badge>)}
                    </div>
                  )}
                  {p.email && <p className="text-xs text-muted-foreground mt-0.5">{p.email}</p>}
                </div>
                <div className="flex gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                  <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditing(p); setOpen(true); }}>✏️</Button>
                  <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { if (confirm("Verwijderen?")) remove.mutate(p.id); }}>🗑️</Button>
                </div>
              </div>
              {expandedId === p.id && (
      ...
      <PersonForm open={open} onOpenChange={setOpen} defaultValues={editing}
        onSubmit={(values) => upsert.mutate(values as unknown as Omit<Person, "id">)} isPending={upsert.isPending} />
    </div>
  );
}
